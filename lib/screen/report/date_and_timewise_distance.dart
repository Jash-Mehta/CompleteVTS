...
class DateAndTimeWiseDistanceScreen extends StatefulWidget {
  ...
  @override
  _DateAndTimeWiseDistanceScreenState createState() => _DateAndTimeWiseDistanceScreenState();
}

class _DateAndTimeWiseDistanceScreenState extends State<DateAndTimeWiseDistanceScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: CustomAppBar()
          .getCustomAppBar("DATE AND TIME WISE DISTANCE", _scaffoldKey, 2, context),
      body: _vehicleMaster(),
    );
  }
    ...
    return LoadingOverlay(
      ...
      child: BlocListener<MainBloc, MainState>(
        listener: (context, state) {
          if (state is DateAndTimeWiseDistanceLoadingState) {
            setState(() {
              _isLoading = true;
            });
          } else if (state is DateAndTimeWiseDistanceLoadedState) {
            setState(() {
              _isLoading = false;
              pageNumber++;
              var totalOverSpeedCreateRecords =
                  state.DateAndTimeWiseDistanceResponse.totalRecords!;

              // data!.addAll(state.getOverspeedREportResponse.data!);
            });
          } else if (state is DateAndTimeWiseDistanceErrorState) {
            setState(() {
              _isLoading = false;
            });
          }
