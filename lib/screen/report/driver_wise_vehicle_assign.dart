...
class _DriverWiseVehicleAssignScreenState
  ...
  List<DriverWiseVehicle> pdfdatalist = [];
  ...
  List<DriverwiseVAData>? filtersearchdata = [];
  ...
  onSearchTextChanged(String text) async {
    if (text.isEmpty) {
      ...
      (applyclicked && isSearch)
          ? _mainBloc.add(DriverwiseVAFilterSearchEvent(
              token: token,
              vendorid: vendorid,
              branchid: 1,
              vsrno: dwdcvehicleno.toString() == null
                  ? "ALL"
                  : dwdcvehicleno.toString(),
    ...
    return LoadingOverlay(
      ...
      child: BlocListener<MainBloc, MainState>(
        listener: (context, state) {
          ...
          if (state is DriverWiseVehicleFilterLoadingState) {
            ...
            if (state.DriverWiseVehicleFilterResponse.data != null) {
              ...
              setState(() {
                _isLoading = false;
                filterdata!.clear();
                value = state.DriverWiseVehicleFilterResponse.totalRecords!;
              });
              filterdata!.addAll(state.DriverWiseVehicleFilterResponse.data!);
            } else {
              setState(() {
          ...
          // filter search
          if (state is DriverWiseVAFilterSearchLoadingState) {
            setState(() {
              _isLoading = true;
            });
          } else if (state is DriverWiseVAFilterSearchLoadedState) {
            if (state.searchvehassignResponse.data != null) {
              setState(() {
                _isLoading = false;
                filtersearchdata!.clear();
              });
              filtersearchdata!.clear();
              filtersearchdata!.addAll(state.searchvehassignResponse.data!);
            } else {
              setState(() {
                filtersearchdata!.clear();
                _isLoading = false;
          ...
          } else if (state is DriverWiseVAFilterSearchErrorState) {
            setState(() {
              _isLoading = false;
            });
          }
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            GestureDetector(
                              onTap: () async {
                                pdfdatalist.clear();
                                pdfdatalist.addAll(driverwisevehicledata!);
                                pdffilterlist.clear();
  ...
  String convertDataToCsv(
    ...
    //   columns: List.generate(data.length, (index) => DataColumn(label: Text(data[index].toString()))),
    //   rows: List.generate(data.length, (index) => DataRow(cells:[DataCell(Text(data[index].toString()))]  )));
  ...
  static Future<File> generate(
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (pw.Context context) {
        return <pw.Widget>[
          pw.Center(
              child: pw.Text("DRIVER WISE VEHICLE REPORT",
                  style: pw.TextStyle(
