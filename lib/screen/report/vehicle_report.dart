...
class _VehicleReportScreenState extends State<VehicleReportScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: CustomAppBar()
          .getCustomAppBar("Vehicle Report", _scaffoldKey, 2, context),
      body: _vehicleMaster(),
    );
  }
    ...
    return LoadingOverlay(
      ...
      child: BlocListener<MainBloc, MainState>(
        listener: (context, state) {
          if (state is VehicleReportLoadingState) {
            setState(() {
              _isLoading = true;
            });
          } else if (state is VehicleReportLoadedState) {
            setState(() {
              _isLoading = false;
              pageNumber++;
              var totalOverSpeedCreateRecords =
                  state.VehicleReportResponse.totalRecords!;
