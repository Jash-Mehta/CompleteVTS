...
// for my drawn routes on the map
  ...
  String googleAPIKey = "AIzaSyCzJ9rnQfwR2O7lfUnJt2UGwNicQP_eTUk";
...
// wrapper around the location API
  ...
  LatLng showLocation = const LatLng(27.7089427, 85.3086209); //location to show in map
  LatLng searchLiveTrackingLocation = const LatLng(27.7089427, 85.3086209); //location to show in map
  ...
  void initState() {
    ...
    setState(() {
      showLocation=LatLng(18.6298, 73.7997);
      getmarkers(0);
    });
    getdata();
    googlemapImage();
    ...
    location!.onLocationChanged.listen((LocationData cLoc) {
      ...
      // if (sourcePinInfo!.location!.latitude == 0 &&
      //     sourcePinInfo!.location!.longitude == 0) {
      showPinsOnMap();
      // }
  ...
  Set<Marker> getmarkers(int flag) {
    if(flag==0){
      _markerlist.add(Marker( //add second marker
      ...
      setState(() {
        for(int i=0;i<liveTrackingDetails!.length;i++){
          _markerlist.add(Marker( //add second marker
            markerId: MarkerId(showLocation.toString()),
            position: LatLng(liveTrackingDetails![i].latitude=="0000000000"  ? 18.6298 :double.parse(liveTrackingDetails![i].latitude!),liveTrackingDetails![i].longitude=="00000000000"  ? 73.7997 : double.parse(liveTrackingDetails![i].longitude!)), //position of marker
            // position: LatLng(/*double.parse(liveTrackingDetails![i].latitude!),double.parse(liveTrackingDetails![i].longitude!)*//*27.7099116, 85.3132343*/18.6298, 73.7997), //position of marker

            infoWindow: InfoWindow( //popup info
                title: liveTrackingDetails![i].driverName,
                snippet: liveTrackingDetails![i].vehicleRegNo,
                onTap: (){
                  print("marker click");
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => BlocProvider(
                              ...
                              child: LiveTrackingDetailsScreen(transactionId:liveTrackingDetails![i].transactionId ,araiNonarai:"nonarai")))
                  );
                }
            ),
            icon: BitmapDescriptor.defaultMarker, //Icon for Marker
        ...
        //   position: LatLng(/*double.parse(liveTrackingDetails![i].latitude!),double.parse(liveTrackingDetails![i].longitude!)*//*27.7099116, 85.3132343*/18.6598, 73.7497), //position of marker
        //   infoWindow: InfoWindow( //popup info
        ...
        //                     return MainBloc(webService: WebService());
        //                   },
        //                   child: LiveTrackingDetailsScreen(transactionId:85680/*liveTrackingDetails![i].transactionId*/ ,))));
        //
        //     }
        //   ),
        //   onTap: (){
      ...
      setState(() {
        for(int i=0;i<liveTrackingFilterResponse.data!.length;i++){
          _markerlist.add(Marker( //add second marker
            markerId: MarkerId(showLocation.toString()),
            position: LatLng(liveTrackingFilterResponse.data![i].latitude=="0000000000"  ? 18.6298 :double.parse(liveTrackingFilterResponse.data![i].latitude!),liveTrackingFilterResponse.data![i].longitude=="00000000000"  ? 73.7997 :double.parse(liveTrackingFilterResponse.data![i].longitude!)), //position of marker
            infoWindow: InfoWindow( //popup info
                  ...
                  Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (_) => BlocProvider(
                              ...
                              child: LiveTrackingDetailsScreen(transactionId:liveTrackingFilterResponse.data![i].transactionId ,araiNonarai:araiNonarai))));
      ...
      setState(() {
        for(int i=0;i<searchliveTrackingResponse.length;i++){
          _searchmarkerlist.add(Marker( //add second marker
            markerId: MarkerId(showLocation.toString()),
            position: LatLng(double.parse(searchliveTrackingResponse[i].latitude!),double.parse(searchliveTrackingResponse[i].longitude!)), //position of marker
            infoWindow: InfoWindow( //popup info
                  ...
                  Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (_) => BlocProvider(
                              ...
                              child: LiveTrackingDetailsScreen(transactionId:searchliveTrackingResponse[i].transactionId ,araiNonarai:"nonarai")))
                  );
                }
            ),
            icon: BitmapDescriptor.defaultMarker, //Icon for Marker
      ...
      setState(() async{
        for(int i=0;i<startLocationResponse.length;i++){
          _searchmarkerlist.add(Marker( //add second marker
            markerId: MarkerId(showLocation.toString()),
            position: LatLng(double.parse(startLocationResponse[i].latitude!),double.parse(startLocationResponse[i].longitude!)), //position of marker
            infoWindow: InfoWindow( //popup info
                  ...
                  Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (_) => BlocProvider(
                              ...
                              child: LiveTrackingDetailsScreen(transactionId:startLocationResponse[i].transactionId! ,araiNonarai:"nonarai")))
                  );
                }
            ),
            icon: await BitmapDescriptor.fromAssetImage(
                const ImageConfiguration(devicePixelRatio: 2.0,size: Size(20,20)),
                startLocationResponse[i].vehicleStatus=='Stop' ? 'assets/stop_car.png' : startLocationResponse[i].vehicleStatus=='Running' ? 'assets/running_car.png': startLocationResponse[i].vehicleStatus=='Idle' ? 'assets/idle_car.png' :'assets/inactive_car.png' )
            /*BitmapDescriptor.defaultMarker*//*BitmapDescriptor.defaultMarkerWithHue(90)*//*LiveLocationStatusIcon==null ? BitmapDescriptor.defaultMarker : LiveLocationStatusIcon!*/, //Icon for Marker
          ));
    ...
    return searchVehicle ? _searchmarkerlist :  _markerlist;
  ...
  Future<BitmapDescriptor> _locationIcon()async{
    return  await BitmapDescriptor.fromAssetImage(
        const ImageConfiguration(devicePixelRatio: 2.0,size: Size(20,20)),
        startLocationResponse[0].vehicleStatus=='Stop' ? 'assets/stopped_truck.png' :'assets/idle_truck.png' );
  }

  _getVehicleStatusWithCount(){
    _mainBloc.add(VehicleStatusWithCountEvents(token: token,vendorId:vendorid,branchId: branchid,araiNonarai: "nonarai" ));
  }
  ...
  getlivetrackingdetail(String status){
    _mainBloc.add(LiveTrackingEvents(token:token,vendorId:vendorid,branchId: branchid,TrackingStatus: status,araiNonarai: "nonarai" ));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: AppBar(
                  ...
                  setState(() {
                    ...
                    for(int i=0;i<list.length;i++){
                      if(onChangeVal==list[i]['vsrNo'].toString()){
                        ...
                        _mainBloc.add(SearchLiveTrackingEvents(token: token,vendorId: vendorid,branchId :branchid,araiNonarai: "nonarai",trackingStatus : "total"/*radioItemHolder1*/,searchText:list[i]['vehicleRegNo']));
          ...
          Text("LIVE TRACKING"),
          actions: [
            searchVehicle ? Container() :GestureDetector(
              onTap: (){
                setState(() {
                  searchVehicle=true;
                });
            ...
            GestureDetector(
              ...
              child: Container(
                margin: EdgeInsets.only(right: 10),
                child:Image.asset("assets/setting.png",height: 40,width: 40,) ,
              ),
            ),
          ],
        ),
        body: _livevehicletracking(initialCameraPosition)/*_livetracking(initialCameraPosition)*/
    );
  }
  ...
  _filterResult() async{
    ...
    if (filterLiveTrackingresult != null && filterLiveTrackingresult.containsKey('SearchFilter')) {
      searchFilter=filterLiveTrackingresult["SearchFilter"];
    }

    if (filterLiveTrackingresult != null && filterLiveTrackingresult.containsKey('SelectedVehicleList')) {
      selectedvehicleSrNolist=filterLiveTrackingresult["SelectedVehicleList"];
    }

    if (filterLiveTrackingresult != null && filterLiveTrackingresult.containsKey('LiveTrackingData')) {
      liveTrackingFilterResponse=filterLiveTrackingresult["LiveTrackingData"];
      setState(() {
        if(liveTrackingFilterResponse.data![0].latitude=="000000000" && liveTrackingFilterResponse.data![0].longitude=="000000000"){
          getmarkers(0);
        }else{
          _markerlist.clear();
          getmarkers(2);
    ...
    if (filterLiveTrackingresult != null && filterLiveTrackingresult.containsKey('araiNonarai')) {
  ...
  _livevehicletracking(CameraPosition initialCameraPosition){
    return LoadingOverlay(
      ...
      child: BlocListener<MainBloc, MainState>(
        listener: (context, state) {
          if (state is LiveTrackingLoadingState) {
            ...
            if(state.liveTrackingResponse.liveTrackingDetails!=null){
              ...
              print("VTSProject live------------------------------");

              liveTrackingDetails=state.liveTrackingResponse.liveTrackingDetails!;
              ...
              if(state.liveTrackingResponse.message!=null){
                ...
                print("VTSProject  1------------${state.liveTrackingResponse.message!}------------------");
            ...
            if(state.vehicleStatusWithCountResponse.length!=0){
              setState(() {
                id1=int.parse(state.vehicleStatusWithCountResponse[0].statusId!);
                radioItemHolder1=state.vehicleStatusWithCountResponse[0].status!;
              });
            }
          ...
          if (state is StartLocationIMEILoadingState) {
            setState(() {
              _isLoading = true;
            });
        ...
        child: Column(
          children: [
            vehicleStatusWithCountResponse.length==0 ? Container() :Expanded(
              flex: 2,
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: Radio(
                              ...
                              value:id1,
                              groupValue: int.parse(vehicleStatusWithCountResponse[i].statusId!),
                              onChanged: (value) {
                                setState(() {
                                  radioItemHolder1 = vehicleStatusWithCountResponse[i].status!;
                                  id1 = int.parse(vehicleStatusWithCountResponse[i].statusId!);
                                  print(radioItemHolder1);
                                  _searchmarkerlist.clear();

                                  getlivetrackingdetail(vehicleStatusWithCountResponse[i].status!);
                                });
                              },

                            ),
                          ),
                          Expanded(child: Text("(${vehicleStatusWithCountResponse[i].count.toString()})",style: TextStyle(fontWeight: FontWeight.bold))),
                          Expanded(
                            child: Padding(
                              padding: const EdgeInsets.only(top:4.0),
                              child: Text(vehicleStatusWithCountResponse[i].status!,style: TextStyle(fontWeight: FontWeight.bold),),
                            ),
                          ),
                        ],
                      ),
            ...
            Expanded(
                ...
                child: Container(
                  ...
                  onMapCreated: (controller) { //method called when map is created
                    ...
                    markers: searchVehicle ? _searchmarkerlist : _markerlist,
  ...
  getstartlocation(String imei){
    ...
    _mainBloc.add(StartLocationIMEIEvents(vendorId: vendorid,branchId: branchid,token: token, araiNonarai: 'nonarai',imeiNUmber:imei ));
  ...
  getNextlocation(String imei){
    ...
    _mainBloc.add(NextLocationIMEIEvents(vendorId: vendorid,branchId: branchid,token: token, araiNonarai: 'nonarai',imeiNUmber:imei ));
